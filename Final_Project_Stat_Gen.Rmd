...
paired_samples <- results %>%
  ...
  mutate(count = n_distinct(sample_type)) %>%
...
## Two Sample and Paired T-Tests: I have this in the previous block
...
```{r}
# For paired
tbVals <- tibble(t(bVals))
sample_id <- rownames(tbVals)
tbVals <- bind_cbind(sample_id=sample_id, tbVals)
...
# Overlap
sig_in_paired <- paired_t_test_res_FDR$CpG_Site[paired_t_test_res_FDR$FDR_P_Value<0.05]
sig_in_2sample <- t_test_res_FDR$CpG_Site[t_test_res_FDR$FDR_P_Value<0.05]
overlap <- intersect(sig_in_paired, sig_in_2sample)
```

## Manhattan Plot of FDR Corrected P-Values from Paired T Test:

```{r}
# Can u change the names according to 'annotations'
names(annotations)
# Name = "probeID"?
# chr = seqnames?
# pos = start?
